...
func TestRevisionCommand_StorageNamespaceFlag(t *testing.T) {
	cmd := revisionCmd()
	f := cmd.Flags()

	if f.Lookup("storage-namespace") == nil {
		t.Fatal("expected flag --storage-namespace to be registered on revisionCmd")
	}

	if f.Lookup("namespace") == nil {
		t.Fatal("expected flag --namespace to be registered on revisionCmd")
	}

	if f.ShorthandLookup("n") == nil {
		t.Fatal("expected shorthand flag -n to be registered on revisionCmd")
	}
...
func TestRevisionCommand_Execution_StorageNamespace(t *testing.T) {
	manifestYAML := `---
apiVersion: v1
kind: ConfigMap
metadata:
...
`
	...
	t.Run("explicit flag passes storage namespace to helm get", func(t *testing.T) {
		...
		cmd := revisionCmd()
		cmd.SetArgs([]string{"my-release", "1", "2", "--storage-namespace", "flux-system", "-n", "prod-apps"})

		err := cmd.Execute()
		if err != nil {
			t.Fatalf("unexpected error executing revision command: %v", err)
		}
		...
		if !strings.Contains(argsContent, "get manifest my-release --revision 1 --namespace flux-system") {
			t.Errorf("expected 'helm get manifest' for revision 1 to use --namespace flux-system, got:\n%s", argsContent)
		}
	...
	t.Run("env var sets storage namespace when flag is omitted", func(t *testing.T) {
		...
		cmd := revisionCmd()
		cmd.SetArgs([]string{"my-release", "1", "2", "-n", "prod-apps"})
		...
		if !strings.Contains(argsContent, "get manifest my-release --revision 1 --namespace flux-system-env") {
	...
	t.Run("defaults to target namespace when storage namespace is omitted", func(t *testing.T) {
		...
		if !strings.Contains(argsContent, "get manifest my-release --revision 1 --namespace prod-apps") {
