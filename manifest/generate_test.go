...
// TestCreatePatchForUnstructured tests the three-way merge implementation for
// unstructured objects (CRDs, CRs). This tests the fix for issue #917 where
// manual changes to CRs were not being detected.
func TestCreatePatchForUnstructured(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			patch, err := createPatch(tt.original, tt.current, tt.target)
			require.NoError(t, err, "createPatch should not return an error")
			require.Equal(t, types.MergePatchType, patch.patchType, "patch type should be MergePatchType for unstructured objects")

			t.Logf("Patch result: %s", string(patch.data))

			if tt.expectedChange {
				require.NotEmpty(t, patch.data, tt.description+": expected patch to detect changes, got empty patch")
				require.NotEqual(t, []byte("{}"), patch.data, tt.description+": expected patch to detect changes, got empty patch")
				require.NotEqual(t, []byte("null"), patch.data, tt.description+": expected patch to detect changes, got null patch")
			} else {
				// No changes expected: patch must be empty or effectively empty ("{}" or "null")
				if len(patch.data) == 0 || string(patch.data) == "{}" || string(patch.data) == "null" {
					return
				}
				require.Failf(t, tt.description+": expected no changes, got unexpected patch", "unexpected patch: %s", string(patch.data))
			}
		})
	}
}
...
func TestCreatePatchForCRD(t *testing.T) {
	...
	patch, err := createPatch(originalCRD, currentCRD, target)
	require.NoError(t, err, "createPatch should not return an error for CRD")
	require.Equal(t, types.MergePatchType, patch.patchType, "patch type should be MergePatchType for CRD objects")

	t.Logf("CRD Patch result: %s", string(patch.data))

	require.Equal(t, "{}", string(patch.data), "CRD with no changes should result in empty patch")
}
